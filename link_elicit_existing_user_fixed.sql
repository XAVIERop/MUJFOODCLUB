-- Link existing ELICIT auth user to the cafe (Fixed version)
...
BEGIN
    ...
    -- Create or update cafe_staff entry (this is how cafe ownership is managed)
...
SELECT 
    ...
    cs.is_active
FROM cafes c
LEFT JOIN profiles p ON p.cafe_id = c.id AND p.user_type = 'cafe_owner'
LEFT JOIN cafe_staff cs ON c.id = cs.cafe_id AND p.id = cs.user_id
