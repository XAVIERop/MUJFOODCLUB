-- Setup Munch Box cafe owner
-- Creates profile, sets permissions, and links to Munch Box cafe

-- Create profile for Munch Box owner
...
SELECT 
  ...
  'G1',
  'cafe_owner',
  (SELECT id FROM cafes WHERE name = 'Munch Box' LIMIT 1),
  NOW(),
  NOW()
FROM auth.users au
WHERE au.email = 'munchbox.owner@mujfoodclub.in'
AND NOT EXISTS (
