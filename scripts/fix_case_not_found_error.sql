-- Fix "case not found" error in order creation
-- The CASE statement in the trigger is missing a default case

-- 1. Drop the problematic trigger
DROP TRIGGER IF EXISTS consolidated_order_update_trigger ON public.orders;

-- 2. Fix the trigger function with proper CASE handling
...
DECLARE
  ...
  item_date DATE;
BEGIN
  -- Only process if status actually changed (for updates) or if it's a new order
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status) THEN
    
    -- Update status timestamps based on new status (only for updates with status changes)
...
SELECT 'Fixed trigger created successfully:' as status;
SELECT 
  trigger_name,
  event_manipulation,
  action_timing,
...
-- 5. Test with a simple order creation (this should not cause errors)
SELECT 'Testing trigger with sample data:' as status;
SELECT 'Trigger should now handle new orders without "case not found" error' as result;
