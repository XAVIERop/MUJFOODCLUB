-- Implement Daily Reset Order Number System (FIXED VERSION)
...
-- 1. First, let's see what cafes we have
SELECT 'Current Cafes:' as info;
SELECT id, name, is_active FROM public.cafes WHERE is_active = true ORDER BY name;

-- 2. Check current order number patterns
SELECT 'Current Order Numbers (Last 10):' as info;
SELECT order_number, created_at, cafe_id FROM public.orders ORDER BY created_at DESC LIMIT 10;

-- 3. Create cafe prefix mapping function
...
-- 4. Create the main order number generation function (FIXED)
...
DECLARE
  ...
  generated_order_number TEXT;  -- Changed variable name to avoid conflict
...
BEGIN
  ...
  LOOP
    ...
    generated_order_number := cafe_prefix || LPAD(new_order_num::TEXT, 6, '0');
    
    -- Double-check uniqueness (extra safety)
    IF NOT EXISTS (
      SELECT 1 FROM public.orders o
      WHERE o.order_number = generated_order_number
    ) THEN
...
-- 5. Test the function with existing cafes
...
-- 6. Create a helper function to validate order numbers
...
-- 8. Show current order count by cafe for today
SELECT 'Current Orders Today by Cafe:' as info;
SELECT 
  c.name as cafe_name,
...
SELECT 'Ready to update frontend code' as next_step;
