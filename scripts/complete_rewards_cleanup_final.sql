-- Complete Rewards Cleanup - Drop ALL Tables First
-- This script drops every possible rewards-related table before dropping types
...
AND (tablename LIKE '%loyalty%' OR tablename LIKE '%rewards%' OR tablename LIKE '%tier%' OR tablename LIKE '%bonus%' OR tablename LIKE '%maintenance%')
ORDER BY tablename;

-- Step 2: Drop ALL possible rewards tables (comprehensive list)
...
DROP TABLE IF EXISTS public.cafe_loyalty_transactions CASCADE;
DROP TABLE IF EXISTS public.user_rewards_summary CASCADE;
DROP TABLE IF EXISTS public.cafe_loyalty_summary CASCADE;
DROP TABLE IF EXISTS public.user_loyalty_summary_view CASCADE;
DROP TABLE IF EXISTS public.cafe_loyalty_summary_view CASCADE;

-- Step 3: Drop any views that might exist
DROP VIEW IF EXISTS public.user_rewards_summary CASCADE;
DROP VIEW IF EXISTS public.cafe_loyalty_summary CASCADE;
DROP VIEW IF EXISTS public.user_loyalty_summary_view CASCADE;
DROP VIEW IF EXISTS public.cafe_loyalty_summary_view CASCADE;

-- Step 4: Now drop the types (should work without table dependencies)
DO $$
BEGIN
...
-- Step 6: Remove rewards columns from profiles table
...
DROP INDEX IF EXISTS idx_tier_maintenance_user_id;
DROP INDEX IF EXISTS idx_user_bonuses_user_id;
DROP INDEX IF EXISTS idx_maintenance_periods_user_id;
DROP INDEX IF EXISTS idx_cafe_loyalty_points_user_id_cafe_id;
