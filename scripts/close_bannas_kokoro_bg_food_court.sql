-- Close Banna's Chowki, Kokoro, BG The Food Cart, and Food Court
...
SELECT 
  ...
  END as status
FROM public.cafes
WHERE LOWER(name) LIKE '%banna%chowki%'
   OR LOWER(name) LIKE '%banna''s chowki%'
   OR LOWER(name) LIKE '%kokoro%'
   OR LOWER(name) LIKE '%koko-ro%'
   OR LOWER(name) LIKE '%bg the food cart%'
   OR LOWER(name) LIKE '%bg-the-food-cart%'
   OR LOWER(name) LIKE '%food court%'
   OR slug IN ('bannas-chowki', 'kokoro', 'bg-the-food-cart', 'food-court')
ORDER BY name;
