...
-- This script creates proper rating calculation based on individual user ratings
...
DECLARE
    test_cafe_id UUID;
    rating_summary RECORD;
BEGIN
    -- Get a cafe with ratings to test
    SELECT cafe_id INTO test_cafe_id 
    FROM cafe_ratings 
    LIMIT 1;
    
    IF test_cafe_id IS NOT NULL THEN
        -- Test the rating summary function
        SELECT * INTO rating_summary 
        FROM get_cafe_rating_summary(test_cafe_id);
        
        RAISE NOTICE 'Test Results for cafe %:', test_cafe_id;
        RAISE NOTICE 'Average Rating: %', rating_summary.average_rating;
        RAISE NOTICE 'Total Ratings: %', rating_summary.total_ratings;
        RAISE NOTICE 'Rating Distribution: %', rating_summary.rating_distribution;
    ELSE
        RAISE NOTICE 'No ratings found to test with';
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'Rating calculation system successfully implemented!';
    RAISE NOTICE 'Each user can now rate a cafe only once';
    RAISE NOTICE 'Average rating is calculated from all individual user ratings';
    RAISE NOTICE 'Ratings are automatically updated when users submit/update ratings';
END $$;
