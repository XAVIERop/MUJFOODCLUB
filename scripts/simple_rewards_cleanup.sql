-- Simple Rewards Cleanup - Step by Step
-- This script removes rewards elements one by one to avoid dependency issues

-- Step 1: Check what rewards types exist
SELECT 
    'EXISTING REWARDS TYPES' as category,
...
-- Step 2: Drop rewards enums one by one (base types only)
DO $$
BEGIN
...
-- Step 3: Verify types are dropped
SELECT 
    'REMAINING TYPES' as category,
    typname as type_name,
    typtype as type_kind
FROM pg_type 
WHERE typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
