-- Setup Cook House cafe owner
-- Creates profile, sets permissions, and links to COOK HOUSE cafe

-- Create profile for Cook House owner
...
SELECT 
  ...
  'G1',
  'cafe_owner',
  (SELECT id FROM cafes WHERE name = 'COOK HOUSE' LIMIT 1),
  NOW(),
  NOW()
FROM auth.users au
