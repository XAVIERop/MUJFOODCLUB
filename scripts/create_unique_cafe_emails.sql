-- Create unique cafe owner emails for each cafe
-- This script creates proper cafe owner profiles with unique emails
...
-- 2. Create unique cafe owner profiles for each cafe
-- We'll create new profiles with unique emails for each cafe
...
-- 3. Delete the old cafe staff records
DELETE FROM public.cafe_staff WHERE user_id = (
  SELECT id FROM public.profiles WHERE email = 'chatkara.owner@mujfoodclub.in' AND user_type = 'cafe_owner'
);

-- 4. Create new cafe staff records with unique owners
